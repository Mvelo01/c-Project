...
void setMarbles(cell* row,const int& z, const char& m){
    ...
    /*Setting marbles with 3'O' and 2'+' */
    if(z==5){
        char pos[2]={0,4};
        for(int i=0;i<2;i++){
            row[pos[i]].setMarble('+');
    ...
    /*Setting marbles with 2'O' and 4'+' */
    if(z==6){
            char pos[4]={0,1,4,5};
            for(int i=0;i<4;i++){
                row[pos[i]].setMarble('+');
...
board::board():woc(0),boc(0){
    ...
    for(int i=0;i<13;i++){
        if(i<7){
            ...
            rows.push_back(new cell[1+12-i]);
            connectHorizontal(rows[i],1+12-i);
            mapCells(rows[i],r--,1+12-i);
            if(i>=7){
                setMarbles(rows[i],1+12-i,'@');
            }else
                setMarbles(rows[i],1+12-i,'+');

        }
    }
    for(int i=1;i<13;i++){  //connect  from row 1 to 13
        if(i<7){
            connectDiagonal(rows[i],rows[i-1],0,1+i-1,true);/*connect the buttom half  diagonal*/
            connectDiagonal(rows[i],rows[i-1],1,1+i-1,true);
        }
        else{
            connectDiagonal(rows[i-1],rows[i],0,1+12-i,false); /* connect the buttom half  diagonal for i = 7 to i = 13*/
            connectDiagonal(rows[i-1],rows[i],1,1+12-i,false);
        }
    }
    cell* linep = rows[0];//start at M7
    while(linep!=nullptr){
        cell* cellp = linep;
        while(cellp!=nullptr){
            for(int i=0;i<6;i++){
...
string board::traverseDiagonal() const{
    map<string,cell*>::const_iterator it = cells.find(string("G1"));
...
void board::refreshOffboardCounts(string& winneR){
    int wOnBoard = 0;
    int bOnBoard = 0;
    ...
    woc = 16 - wOnBoard; //*****number of white marbles in the board *********
    boc = 16 - bOnBoard; //*****number of black marbles in the board *********

    if(boc == 6 && woc <6){
    	cout<<"The winner is white";
    }
    else if(woc == 6 && boc < 6){
    	cout << "The winner is back";
    }
}
